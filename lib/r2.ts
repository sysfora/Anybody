...
/**
 * Rewrite absolute paths to work with the /p/{projectId}/ prefix
 * 
 * This converts paths like /assets/file.js to /p/{projectId}/assets/file.js
...
export async function fetchFileFromR2(
  ...
  try {
    ...
    // AWS SDK v3 error handling - check for NoSuchKey in various ways
    if (error && typeof error === 'object') {
      ...
      // Check error code (AWS SDK v3 sometimes uses Code)
      if (errorObj.Code === 'NoSuchKey' || errorObj.Code === 'NotFound') {
        throw new Error('File not found');
      }
      
      // Check $metadata for status code 404
    ...
    // Re-throw original error if not a "not found" error
    throw error;
  }
}
...
export async function checkFileExists(
  ...
  try {
    ...
    // AWS SDK v3 error handling - check for NoSuchKey/NotFound in various ways
    if (error && typeof error === 'object') {
      const errorObj = error as Record<string, unknown>;
